function VaR=GammaNormalVaR(TimeSerie,Zrates,vertex,spread,instrument,portfolio,DeltaT,c)
...
%
%OUTPUT: 
%VaR= value at risk computed with gamma normal method
...
%% Sensitivities with respect to CS01,PV01
   ...
   [delta_ir,Gamma_ir]=ComputeSensitivity(Zrates,spread,instrument,portfolio,Zrates(:,1)',"PV01");
   [delta_cs,Gamma_cs]=ComputeSensitivity(Zrates,spread,instrument,portfolio,spread(:,1)',"CS01");
  
%% Compute Loss and make Q-Q plot:

   Loss=-DF_t*delta-delta_ir*deltaZrates'-delta_cs*deltaSpread'-1/2*(Gamma_ir*(deltaZrates.^2)'+Gamma_cs*(deltaSpread.^2)');
   qqplot(Loss)
 %% Compute VaR
